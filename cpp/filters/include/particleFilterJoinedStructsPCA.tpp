...
particleFilterJoinedStructsPCA<TNClasses>::particleFilterJoinedStructsPCA()
: particleFilterJoinedStructsPCA(0,0,0,0.0,0.0,"","","",std::vector<std::string>(),nullptr,nullptr)
{
...
// Full constructor
template<int TNClasses>
particleFilterJoinedStructsPCA<TNClasses>::particleFilterJoinedStructsPCA(const int ysize, const int xsize, const size_t n_particles, const double radius, const double frame_rate,
...
: particleFilterBase < stateModelPosClassOri<TNClasses> ,
...

{
	...
	std::get<2>(this->state_models) = stateModelSubstructuresPCA<TNClasses,0,1,0> (ysize,xsize,def_file_substructs,radius,subs_names,subs_valid_mask);
...
template<int TNClasses>
void particleFilterJoinedStructsPCA<TNClasses>::visualiseOriPhase(cv::Mat_<cv::Vec3b>* const disp) const
{
...
template<int TNClasses>
void particleFilterJoinedStructsPCA<TNClasses>::visualiseSubstructures(cv::Mat_<cv::Vec3b>* const disp) const
{
	const int n_structures = std::get<2>(this->state_models).getNumStructures();
	for(int s = 0 ; s < n_structures; ++s)
	{
		for(unsigned p = 0; p < this->n_particles; ++p)
		{
			const int heart_class = std::get<0>(this->particles[p]).c;
			if(std::get<2>(this->state_models).structInView(s,heart_class-1))
			{
				const int x = std::round(std::get<2>(this->particles[p]).x[s]*(float(disp->rows)/float(this->ysize)));
				const int y = std::round(std::get<2>(this->particles[p]).y[s]*(float(disp->rows)/float(this->ysize)));
				const structVisible_enum visible = std::get<2>(this->particles[p]).visible[s];
